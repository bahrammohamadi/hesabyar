...
export function PortalMenu({ anchorRef, open, onClose, children }: { anchorRef: RefObject<HTMLElement>; open: boolean; onClose: () => void; children: ReactNode }) {
  ...
  return createPortal(
    <>
      ...
      <div className="fixed z-[1260] rounded-2xl border border-border bg-card p-2 shadow-2xl animate-pop-in" style={{ top: style.top, left: style.left, width: style.width }} dir="rtl">
        {children}
      </div>
    </>,
    document.body
