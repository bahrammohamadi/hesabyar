    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-3 mb-5">
        {AUTOMATIONS.map((item) => {
          ...
          return <button key={item.id} onClick={() => setActive(item.id)} className={`card p-4 text-right hover:border-brand-300 transition ${active === item.id ? "border-brand-400 bg-brand-50" : ""}`}><div className="flex items-center gap-2 mb-2"><Icon size={18} className="text-brand-600" /><span className="font-bold text-slate-800">{item.title}</span></div><div className="text-xs text-slate-500 leading-5">{item.hint}</div><div className="text-lg font-bold text-brand-700 mt-3">{toFaDigits(count)}</div></button>;
        })}
      </div>
      <div className="card p-4 mb-4"><div className="flex items-center gap-2"><ActiveIcon size={18} className="text-brand-600" /><div><h2 className="font-bold text-slate-800">{activeDef.title}</h2><p className="text-xs text-slate-400 mt-1">{activeDef.hint}</p></div></div></div>
...
function CampaignCard({ row, onFollowup }: { row: CampaignRow; onFollowup: (row: CampaignRow) => void }) {
  ...
  return <div className="card p-4 flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex items-center gap-2 flex-wrap"><EntityLink type="contact" id={row.contact.id}>{row.contact.name}</EntityLink><span className="badge bg-brand-100 text-brand-700">{row.reason}</span></div><div className="text-sm text-slate-500 mt-2">{row.message}</div><div className="text-xs text-slate-400 mt-1 flex flex-wrap gap-3">{row.contact.phone && <PhoneLink phone={row.contact.phone} />}{row.lastDate && <span>آخرین خرید: {toJalali(row.lastDate)}</span>}{typeof row.total === "number" && <span>خرید: {formatToman(row.total, false)}</span>}{typeof row.walletCredit === "number" && <span>اعتبار: {formatToman(row.walletCredit, false)}</span>}</div></div><div className="flex items-center gap-2 shrink-0 flex-wrap justify-end">{row.contact.phone && <a href={sms} className="btn-secondary text-sm">پیامک</a>}{phone && <a href={whatsapp} target="_blank" rel="noreferrer" className="btn-secondary text-sm">واتساپ</a>}<button onClick={() => onFollowup(row)} className="btn-primary text-sm"><Plus size={14} /> ثبت پیگیری</button><EntityActionMenu type="contact" id={row.contact.id} label={row.contact.name} phone={row.contact.phone} /></div></div>;
}
