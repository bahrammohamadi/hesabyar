    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-3 mb-5">
        {AUTOMATIONS.map((item) => {
          ...
          return <button key={item.id} onClick={() => setActive(item.id)} className={`card p-4 text-right hover:border-primary/30 transition ${active === item.id ? "border-primary/40 bg-primary/[0.06]" : ""}`}><div className="flex items-center gap-2 mb-2"><Icon size={18} className="text-primary" /><span className="font-bold text-foreground">{item.title}</span></div><div className="text-xs text-muted-foreground leading-5">{item.hint}</div><div className="text-lg font-bold text-primary mt-3">{toFaDigits(count)}</div></button>;
        })}
      </div>
      <div className="card p-4 mb-4"><div className="flex items-center gap-2"><ActiveIcon size={18} className="text-primary" /><div><h2 className="font-bold text-foreground">{activeDef.title}</h2><p className="text-xs text-muted-foreground mt-1">{activeDef.hint}</p></div></div></div>
...
function CampaignCard({ row, onFollowup }: { row: CampaignRow; onFollowup: (row: CampaignRow) => void }) {
  ...
  return <div className="card p-4 flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex items-center gap-2 flex-wrap"><EntityLink type="contact" id={row.contact.id}>{row.contact.name}</EntityLink><span className="badge bg-primary/10 text-primary">{row.reason}</span></div><div className="text-sm text-muted-foreground mt-2">{row.message}</div><div className="text-xs text-muted-foreground mt-1 flex flex-wrap gap-3">{row.contact.phone && <PhoneLink phone={row.contact.phone} />}{row.lastDate && <span>آخرین خرید: {toJalali(row.lastDate)}</span>}{typeof row.total === "number" && <span>خرید: {formatToman(row.total, false)}</span>}{typeof row.walletCredit === "number" && <span>اعتبار: {formatToman(row.walletCredit, false)}</span>}</div></div><div className="flex items-center gap-2 shrink-0 flex-wrap justify-end">{row.contact.phone && <a href={sms} className="btn-secondary text-sm">پیامک</a>}{phone && <a href={whatsapp} target="_blank" rel="noreferrer" className="btn-secondary text-sm">واتساپ</a>}<button onClick={() => onFollowup(row)} className="btn-primary text-sm"><Plus size={14} /> ثبت پیگیری</button><EntityActionMenu type="contact" id={row.contact.id} label={row.contact.name} phone={row.contact.phone} /></div></div>;
}
...
function FollowupModal({ row, onClose }: { row: CampaignRow; onClose: () => void }) {
  ...
  return <Modal open onClose={onClose} title="ثبت پیگیری کمپین" size="md"><div className="space-y-4"><div className="rounded-xl bg-muted p-3"><EntityLink type="contact" id={row.contact.id}>{row.contact.name}</EntityLink><div className="text-xs text-muted-foreground mt-1">{row.reason}</div></div><div><label className="label">عنوان</label><input aria-label="عنوان" className="input" value={title} onChange={(e) => setTitle(e.target.value)} /></div><div><label className="label">متن پیام/پیگیری</label><textarea className="input" rows={4} value={description} onChange={(e) => setDescription(e.target.value)} /></div><div><label className="label">تاریخ پیگیری بعدی</label><DatePicker value={nextDate} onChange={setNextDate} /></div>{error && <div className="rounded-xl bg-destructive/10 text-destructive-text p-3 text-sm">{error}</div>}<div className="flex gap-2"><button onClick={save} className="btn-primary flex-1">ثبت</button><button onClick={onClose} className="btn-secondary">انصراف</button></div></div></Modal>;
}
