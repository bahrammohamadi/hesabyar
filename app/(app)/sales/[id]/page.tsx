      ...
      <Modal open onClose={onClose} title="ویرایش فاکتور فروش" size="lg" mobileFullscreen>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 border-t border-border pt-4">
            ...
            <div><label className="label">مالیات (تومان)</label><input aria-label="مالیات (تومان)" className="input" inputMode="numeric" value={tax} onChange={(e) => setTax(e.target.value)} /></div>
...
function CancelSaleModal({ saleId, invoiceNo, onClose }: { saleId: string; invoiceNo?: string | null; onClose: () => void }) {
  ...
  async function cancel() {
    if (demoGuard("ابطال فاکتور")) return;
    setSaving(true);
    const supabase = createClient();
    try {
  ...
  return (
    <Modal open onClose={onClose} title="ابطال فاکتور فروش" size="md">
      <div className="space-y-4">
        <div className="rounded-xl bg-destructive/10 text-destructive-text p-3 text-sm">فاکتور {invoiceNo ?? ""} باطل می‌شود؛ موجودی اقلام برمی‌گردد و پرداخت‌ها خنثی می‌شوند.</div>
