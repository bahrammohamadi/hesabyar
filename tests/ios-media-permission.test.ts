...
describe("🔴 iOS: مسیر مجوز جدا از getUserMedia", () => {
  const voice = readCode("components/shared/voice-order.tsx");
  const platform = read("lib/utils/platform.ts");

  it("در iOS اول getUserMedia صدا زده نمی‌شود", () => {
    /*
      گزارش کاربر با اسکرین‌شات: «بارکدخوان و دوربین کار می‌کند ولی
      میکروفون نه». همین تفاوت ریشه را لو داد.

      در سافاری iOS، getUserMedia({audio}) و SpeechRecognition دو
      مسیر مجوز جداگانه دارند. اولی می‌تواند رد شود در حالی که دومی
      کار می‌کند. ما با رد شدن اولی زودتر تسلیم می‌شدیم و «دسترسی
      داده نشد» می‌گفتیم — بدون اینکه اصلاً تشخیص گفتار را امتحان
      کرده باشیم.
    */
    expect(voice).toContain("if (!ios && navigator.mediaDevices?.getUserMedia)");
  });

  it("در غیر iOS همچنان getUserMedia مقدم است", () => {
    // کروم بدون آن بی‌صدا شکست می‌خورد.
    expect(voice).toContain("getUserMedia({ audio: true })");
  });

  it("🔴 راهنما دیگر «بالای صفحه» نمی‌گوید", () => {
    /*
      از iOS 15 نوار نشانی سافاری پیش‌فرض **پایین** صفحه است.
      اسکرین‌شات کاربر همین را نشان داد؛ او دنبال دکمه‌ای در بالا
      می‌گشت که آنجا نبود.
    */
    expect(platform).not.toContain("نوار نشانی بالای صفحه");
    expect(platform).toContain("پایین صفحه");
  });

  it("آیکون خط‌خورده به‌عنوان کوتاه‌ترین راه معرفی می‌شود", () => {
    // وقتی سافاری مسدود می‌کند، همان آیکون مستقیم به تنظیمات می‌برد.
    expect(platform).toContain("خط‌خورده");
  });

  it("راه خروج از بن‌بست وجود دارد", () => {
    // «هنوز تو آیفون نمی‌تونم کاری کنم» — کاربر نباید حبس شود.
