...
export function CoreRuntimeDevButton() {
  ...
  return (
    <div className="fixed bottom-28 right-4 z-[70] flex flex-col items-start gap-2 rounded-2xl border border-slate-200 bg-white/95 p-3 text-right shadow-xl backdrop-blur" dir="rtl">
      <div className="flex items-center gap-2 text-xs font-bold text-slate-600">
        <Layers3 size={15} /> Core Runtime PoC · Stack: {stack.length}
      </div>
      {lastSelection && <div className="max-w-56 truncate text-[11px] text-slate-400">آخرین انتخاب: {lastSelection}</div>}
      <button
